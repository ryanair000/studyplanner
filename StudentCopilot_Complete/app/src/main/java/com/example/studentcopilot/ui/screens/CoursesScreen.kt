        ...
        if (courses.isEmpty()) {
            ...
            LazyColumn(
                ...
                items(courses) { course ->
                    CourseCard(
                        name = course.name,
                        onClick = { /* Navigate to course details if needed */ }
                    )
                }
            }
        }
...
// Helper extension for AddButton with custom modifier
@Composable
fun AddButton(onClick: () -> Unit, modifier: Modifier = Modifier) {
    com.example.studentcopilot.ui.components.AddButton(onClick = onClick)
}
